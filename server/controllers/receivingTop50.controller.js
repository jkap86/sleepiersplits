    ...
    try {
        ...
        const total = await Plays.findAll({
            attributes: [
                ...
                [Sequelize.fn('COUNT', Sequelize.col('players.playerPlays.playId')), 'routes']
            ],
            include: [{
                model: db.sequelize.model('players'),
                attributes: [],
                through: { attributes: [] }
            }],
            where: {
            ...
            group: ['receiver_player_id', Sequelize.col('players.playerPlays.playId')],
            raw: true
        })
        ...
        for (const wr of total.filter(t => top_ids.includes(t.receiver_player_id))) {
            const routes = await Plays.count({
                where: {
                    [Op.and]: [
                        {
                            offense_players: { [Op.contains]: [wr.receiver_player_id] }
                        },
                        {
                            pass_attempt: "1"
                        },
